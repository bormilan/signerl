...
- `xades:SignedProperties` under `ds:Signature`.
...
- Signature validation now also covers signed-properties integrity in the project-local deterministic profile.

## What Is Still Missing (After This Iteration)
- XML canonicalization (C14N/C14N11/Exclusive C14N).
- Full XMLDSig `SignedInfo` + `Reference` model for standards interoperability.
...
- This iteration gives internal deterministic signed-properties support.
- It does **not** yet guarantee interoperability with third-party XMLDSig/XAdES validators until canonicalization and standards-aligned reference processing are added.
